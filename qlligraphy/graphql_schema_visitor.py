...
def visit_type_definition_node(
    visitor: Visitor[Node, AST], node: ObjectTypeDefinitionNode
) -> ClassDef:
    class_body = cast(List[stmt], [visitor.visit(field) for field in node.fields])
...
def visit_field_definition_node(
    visitor: Visitor[Node, AST], node: FieldDefinitionNode
) -> AnnAssign:
    target = visitor.visit(node.name)
    annotation: Union[AST, Subscript] = visitor.visit(node.type)
...
def visit_non_null_type_node(visitor: Visitor[Node, AST], node: NonNullTypeNode):
    return visitor.visit(node.type)


@graphql_schema_visitor.register(NamedTypeNode)
def visit_named_type_node(visitor: Visitor[Node, AST], node: NamedTypeNode):
    return visitor.visit(node.name)


@graphql_schema_visitor.register(ListTypeNode)
def visit_list_type_node(visitor: Visitor[Node, AST], node: ListTypeNode):
...
def visit_name_node(_: Visitor[Node, AST], node: NameNode) -> Name:
